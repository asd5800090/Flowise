...
const AddDocStoreDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
    ...
    const createDocumentStore = async () => {
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('message.newDocumentStoreCreated'),
                    options: {
            ...
            enqueueSnackbar({
                message: t('message.failedToAddDocumentStore', { error: errorData }),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
    ...
    const updateDocumentStore = async () => {
        try {
            ...
            if (saveResp.data) {
                enqueueSnackbar({
                    message: t('message.documentStoreUpdated'),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
            ...
            enqueueSnackbar({
                message: t('message.failedToUpdateDocumentStore', { error: errorData }),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
