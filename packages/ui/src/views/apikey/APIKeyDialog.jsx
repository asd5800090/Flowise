...
const APIKeyDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    const addNewKey = async () => {
        try {
            ...
            enqueueSnackbar({
                message: t('apikey.addFailed', { error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data }),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
    ...
    const saveKey = async () => {
        try {
            ...
            if (setError) setError(error)
            enqueueSnackbar({
                message: t('apikey.saveFailed', { error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data }),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
