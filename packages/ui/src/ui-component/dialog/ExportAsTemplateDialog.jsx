...
const ExportAsTemplateDialog = ({ show, dialogProps, onCancel }) => {
    ...
    useEffect(() => {
        if (dialogProps.chatflow) {
            ...
            setFlowType(dialogProps.chatflow.type === 'MULTIAGENT' ? t('exportTemplate.agentflow') : t('exportTemplate.chatflow'))
        }

        if (dialogProps.tool) {
            setName(dialogProps.tool.name)
            setDescription(dialogProps.tool.description)
            setFlowType(t('exportTemplate.tool'))
        }
    ...
    const onConfirm = () => {
        if (name.trim() === '') {
            enqueueSnackbar({
                message: t('exportTemplate.nameMandatory'),
                options: {
            ...
            <DialogContent>
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                            <Typography sx={{ mb: 1 }}>
                            {t('exportTemplate.name')}<span style={{ color: 'red' }}>&nbsp;*</span>
                        </Typography>
                        <OutlinedInput
                            id={'name'}
                            type={'string'}
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        ...
                        <Typography variant='body2' sx={{ fontStyle: 'italic', mt: 1 }} color='text.secondary'>
                            {t('exportTemplate.usecaseHint')}
                        </Typography>
                    </div>
                </Box>
            </DialogContent>
            <DialogActions>
                <Button onClick={onCancel}>{dialogProps.cancelButtonName || t('button.cancel')}</Button>
